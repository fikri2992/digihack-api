...
class AuthController {
  /*
	...
	*/
	...
	/*
		Check disposible email
	*/
	async disposableEmail({ request, response }) {
		const result = await AuthService.disposableEmail(request);
		return response.status(result.status).send(result.data);
	}
	...
	*/
	async checkLogin({ request, response, auth }) {
		const result = await AuthService.checkLogin(request, auth);
		return response.status(result.status).send(result.data);
	}
   ...
   */
	...
	*/
	async register({ response, request }) {
		...
		const result = await AuthService.userRegister(request);
		return response.status(result.status).send(result.data);
	}
