...
class ExceptionHandler extends BaseExceptionHandler {
  ...
  async handle (error, { response }) {
    ...
    switch (error.name) {
      ...
      case 'ValidationException': return response.status(400).send({ message: error.message[0].message })
    }
    ...
    if (Env.get('NODE_ENV') === 'development') {
      ...
      return response.status(error.status).send({ message: error.message, errorName: error.name, errorCode: error.code })
    }
